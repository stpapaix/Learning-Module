...
namespace learning_module
{
    ...
    class Program

     {
        ...
        private async Task<User> ReadUserDocument(string databaseName, string collectionName, User user)
        {
            
            // Read document and return User 
            try
            {
                await this.client.ReadDocumentAsync(UriFactory.CreateDocumentUri(databaseName, collectionName, user.Id), new RequestOptions { PartitionKey = new PartitionKey(user.UserId) });
        ...
        private async Task SoftDeleteUserDocumentWithTTL(string databaseName, string collectionName, User deletedUser, int myTTLValue)
        {
           try
            ...
            catch (DocumentClientException de)
            {
                if (de.StatusCode == HttpStatusCode.NotFound)
                {
                    this.WriteToConsoleAndPromptToContinue("User {0} not found for replacement", deletedUser.Id);
                }
                else
                {
                    throw;
